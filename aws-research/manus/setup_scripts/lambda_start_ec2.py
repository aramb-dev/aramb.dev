...
# AWS Lambda Function to Start EC2 Instances
# This function starts the EC2 instances specified in the INSTANCE_IDS environment variable
...
def lambda_handler(event, context):
    ...
    try:
        print(f"Attempting to start instances: {instance_ids} in region {region}")
        ec2.start_instances(InstanceIds=instance_ids)
        print(f"Successfully initiated start for instances: {instance_ids}")
        return {
            "statusCode": 200,
            "body": f"Successfully initiated start for instances: {instance_ids}"
        }
